...
const wss = new WebSocketServer({ port: config.port_3 });
...
function Idle() {
  md5_code = crypto.createHash("md5").update(lastHash).digest("hex");
...
function hmac(key, v) {
  const hmacHasher = crypto.createHmac('sha256', key);
  return hmacHasher.update(v).digest("hex");
}
