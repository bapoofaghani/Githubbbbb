						...
						<div class="col-lg-9">
							<table class="table nopointer">
								...
								{foreach $betRecord->bet_form as $val}
									{$bet_form_result = file_get_contents('https://api.soccerama.pro/v1.2/matches/'|con:'':$val->match_id:'/?api_token=S4cqZDiQFqDfIXfLCD7CLne7QC0SqcmbcnufFSwWxWhRruoJkUBa7fTeKMjT&include=homeTeam,awayTeam')}
									{if empty($bet_form_result)}
										{$bet_form_result = file_get_contents('https://soccer.sportmonks.com/api/v2.0/fixtures/'|con:'':$val->match_id:'/?api_token=S4cqZDiQFqDfIXfLCD7CLne7QC0SqcmbcnufFSwWxWhRruoJkUBa7fTeKMjT&include=localTeam,visitorTeam')}
									{/if}
