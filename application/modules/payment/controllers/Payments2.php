...
class Payments extends Public_Controller {
	...
	public function credit() {
		...
		$companyname = 'parsbet.xyz';
		$interface_url = "http://parsbet.xyz";
		...
		if ( $this->formValidate(FALSE) ) {
			...
			if ($type == '1'){
				...
				$dbs = new DB();
				$getway_crown = $dbs->multi_select('gateway' , '*' , ['paymethodid' , 'status'] , [1 ,1]);
				...
				$amount			= $amount;
				$CallbackURL 	= $getway_crown[0]['domain'] . '/payment/verifySezar?transid=' . $trans_id;

				Transaction::create(array('trans_id' => $trans_id, 'price' => $amount, 'invoice_type' => 1, 'cash' => $cash + $amount, 'user_id' => $this->user->id, 'description' => 'افزایش موجودی حساب توسط درگاه کرون'));

				$curl = curl_init();
				curl_setopt($curl, CURLOPT_URL, 'https://crowngate.ir/webservice/paymentRequest.php');
				curl_setopt($curl, CURLOPT_HTTPHEADER, array('Content-Type' => 'application/json'));
				curl_setopt($curl, CURLOPT_POSTFIELDS, "MerchantID={$crown_merchentID}&Amount={$amount}&Description=Crown{$trans_id}&InvoiceNumber={$trans_id}&CardNumber=&CallbackURL=". urlencode($CallbackURL));
				...
				{
					header("Location: https://crowngate.ir/startPay/{$result->Authority}");
					exit;
				
					Transaction::where('trans_id', $_POST['PAYMENT_ID'])->update(array('status' => '1'));
					$cash = $this->__getUserCash();
					$this->sentinel->getUserRepository()->where('id', $this->user->id)->update(array('cash' => $cash + $amount));
			...
			elseif($type == '42'){
				...
				//header("Location:".$redirect);
echo('<form name=\'myForm\' action=\'https://perfectmoney.is/api/step1.asp\' method=\'POST\'><input type=\'hidden\' name=\'PAYEE_ACCOUNT\' value=\'U20378528\'><input type=\'hidden\' name=\'PAYEE_NAME\' value=\'perfect_bet\'><input type=\'hidden\' name=\'PAYMENT_ID\' value=\'' .$transid. '\'><input type=\'hidden\' name=\'PAYMENT_AMOUNT\' value=\'' . $amount . '\'><input type=\'hidden\' name=\'PAYMENT_UNITS\' value=\'USD\'><input type=\'hidden\' name=\'STATUS_URL\' value=\'' . site_url('payment/credit') . '\'><input type=\'hidden\' name=\'PAYMENT_URL\' value=\'' . site_url('payment/credit') . '\'><input type=\'hidden\' name=\'PAYMENT_URL_METHOD\' value=\'POST\'><input type=\'hidden\' name=\'NOPAYMENT_URL\' value=\'' . site_url('payment/credit') . '\'><input type=\'hidden\' name=\'NOPAYMENT_URL_METHOD\' value=\'POST\'><input type=\'hidden\' name=\'SUGGESTED_MEMO\' value=\'\'><input type=\'hidden\' name=\'BAGGAGE_FIELDS\' value=\'IDENT\'><br></form><script type=\'text/javascript\'>window.onload = function(){document.forms[\'myForm\'].submit();}</script>');exit();
				//var_dump($redirect);
				die();
