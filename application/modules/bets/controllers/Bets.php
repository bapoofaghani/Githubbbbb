...
class Bets extends Public_Controller{
	...
	public function inplayBets(){
		...
		//     file_put_contents(APPPATH . 'logs/API/inplay_api.txt', file_get_contents('http://sky7bet.tech/application/logs/API/inplay_api.txt'));
		// }
		// $matches = file_get_contents(APPPATH . 'logs/API/inplay_api.txt');
		// $matches = json_decode($matches, false);
		$matches = json_decode(file_get_contents(API_DIR . "matches.json"));
		$output = array();
		...
		$leagues_entered = file_get_contents($leagues_entered);
